/*
      The IRON-HID Keyboard Firmware for Atmega16U2 in Arduino Mega 2560       
                       Written Han SeungHun, 2014
                  IRON.HID.Project [at] gmail [dot] com 
 
  This firmware is based on LUFA Library and CDEMU project, and you can use it
  for the wired usb type keyboards and the wirless type(bluetooth) keyboards. 

  You can upload this firmware to atmega16U2 with DFU mode. And you should 
  also upload arduino firmware to atmega2560 with Sketch IDE.

  For detail information, visit https://github.com/IRON-HID/Keyboard
*/
                                 ...
                                 uint16_t TotalBlocks)
{
	...
	{		
		...
		for (j = 0 ; j < 512 / sizeof(vcRecvBuffer); j++)
		{
			// Send Start
			Serial_TxByte('s');
			...
			for (i = 0 ; i < sizeof(vcRecvBuffer) ; i++)
			{
				...
				Endpoint_Write_Byte(vcRecvBuffer[i]);
