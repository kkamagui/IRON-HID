...
extern "C" {
	...
	typedef BOOLEAN (__stdcall *HidD_GetHidGuid_)(GUID* interfaceGuid);
...
{
	HANDLE handle;
	DWORD desired_access = (enumerate)? 0:0xC0000000;//(GENERIC_WRITE | GENERIC_READ);
	...
	printf("%X, %d, %d, %d, %d, %d\n", desired_access, share_mode, 0, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, FILE_ATTRIBUTE_NORMAL);
...
struct hid_device_info HID_API_EXPORT * HID_API_CALL hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	printf("%X", DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
	...
	for (;;) {
		...
		*/
		...
		buf[0] = 0x2;
		res = my_hid_get_feature_report(write_handle, buf, sizeof(buf));
		res = my_hid_write(write_handle, buf, sizeof(buf));
		*/
